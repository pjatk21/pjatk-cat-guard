...
class Join(SlashSubCommand):
    ...
    studying_mode: str = Option(
        name="tryb", description="...", choices=("stacjonarnie", "zaocznie")
    )
    group_type: str = Option(
        name="grupa", description="...", choices=("lektorat", "ćwiczenia")
    )
    ...
    async def callback(self, context: SlashCommandContext):
        group_code = create_group_code(
            context.options.group_number, context.options.group_type
        )
        ...
        assigned_thread = group_threads.find_one(
            {
                ...
                "code": group_code,
                "mode": context.options.studying_mode,
            }
        )
