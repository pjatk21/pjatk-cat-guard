...
class CheckUser(Subscription):
    ...
    async def callback(event: GuildMessageCreateEvent):
        ...
        verification = db['verified'].find_one({'discord_id': event.author.id, 'guild_id': event.get_guild().id})

        if verification is None:
            verification_reminder = db['reminders'].find_one(
                {'discord_id': event.author.id, 'guild_id': event.get_guild().id, 'topic': 'verification'}
            )
            ...
            if verification_reminder is None:
                ...
                db["reminders"].insert_one(
                    {'discord_id': event.author.id, 'guild_id': event.get_guild().id, 'topic': 'verification', 'when': datetime.now()}
                )
