...
async def update_tags(event: GuildMessageCreateEvent):
    if event.message.referenced_message and event.message.content:
        ref = await plugin.bot.rest.fetch_message(event.message.referenced_message.channel_id, event.message.referenced_message.id)
...
async def find_arc(ctx: Context):
    ...
    if len(results):
        def create_embed(r: CommonRepoFile):
            return Embed(title=r.file_name, url=r.file_url, timestamp=r.added.astimezone(), color=FILE).add_field(
                'tags', ', '.join(r.tags)
            ).add_field(
